...
pairs(emmeans(glm_disp_svl_inter,~sex|snakes))
...
```{r veusz: body size}
mean_geckos<-aggregate(data_geckos$svl,by=list(data_geckos$snakes,data_geckos$sex),function(x) mean(x,na.rm=T))
mean_lizards<-aggregate(data_lizards$svl,by=list(data_lizards$snakes,data_lizards$sex),function(x) mean(x,na.rm=T))
mean_skinks<-aggregate(data_skinks$svl,by=list(data_skinks$snakes,data_skinks$sex),function(x) mean(x,na.rm=T))
sd_geckos<-aggregate(data_geckos$svl,by=list(data_geckos$snakes,data_geckos$sex),function(x) sd(x,na.rm=T))
sd_lizards<-aggregate(data_lizards$svl,by=list(data_lizards$snakes,data_lizards$sex),function(x) sd(x,na.rm=T))
sd_skinks<-aggregate(data_skinks$svl,by=list(data_skinks$snakes,data_skinks$sex),function(x) sd(x,na.rm=T))
fig_bodysize<-cbind(mean_geckos,sd_geckos[,3],mean_lizards[,3],sd_lizards[,3],mean_skinks[,3],sd_skinks[,3])
colnames(fig_bodysize)<-c("snakes","sex","mean_geckos","sd_geckos","mean_lizards","sd_lizards","mean_skinks","sd_skinks")
...
lapply(models,function(x) pairs(emmeans(x,~snakes|sex)))
lapply(models,function(x) pairs(emmeans(x,~sex|snakes)))

write.csv(results_geckos,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Table 1/results_geckos.csv")
...
morphology<-morphology[,-1]
factors<-data_lizards[-c(29,30),c(1:7)]
morphology<-merge(factors,morphology,by="row.names",all=T)
morphology<-morphology[,-1]
...
lapply(data_lizards[,c(8:15)],function(x) plotmeans(x~site,data=data_lizards))
lapply(data_lizards[,c(8:15)],function(x) plotmeans(x~snakes,data=data_lizards))

models<-lapply(data_lizards[,c(8:15)],function(x) glmmTMB(log(x)~log(svl)+sex*snakes+(1|site),data=data_lizards,dispformula = ~site))
lapply(models,function(x) plot(simulateResiduals(x,n=1000)))
lapply(models,function(x) Anova(x,type=2))
...
write.csv(results_lizards,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Table 1/results_lizards.csv")
...
lapply(models[c(4,7)],function(x) pairs(emmeans(x,~sex|snakes)))

results_skinks<-lapply(models,function(x) Anova(x,type=2))
results_skinks<-do.call(rbind,results_skinks)
write.csv(results_skinks,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Table 1/results_skinks.csv")
...
```{r figure morphology}
geckos_mean<-aggregate(data_geckos$lhl,by=list(data_geckos$snakes,data_geckos$sex),function(x) mean(x,na.rm=T))
aggregate(data_geckos$lamella,by=list(data_geckos$snakes,data_geckos$sex),function(x) mean(x,na.rm=T))
aggregate(data_geckos$lamella,by=list(data_geckos$snakes,data_geckos$sex),function(x) sd(x,na.rm=T))
geckos_sd<-aggregate(data_geckos$lhl,by=list(data_geckos$snakes,data_geckos$sex),function(x) sd(x,na.rm=T))
geckos<-cbind(geckos_mean,geckos_sd)
skinks_mean_sex<-aggregate(data_skinks[,c(11,14)],by=list(data_skinks$snakes,data_skinks$sex),function(x) mean(x,na.rm=T))
skinks_sd_sex<-aggregate(data_skinks[,c(11,14)],by=list(data_skinks$snakes,data_skinks$sex),function(x) sd(x,na.rm=T))
skinks_sex<-cbind(skinks_mean_sex,skinks_sd_sex)
skinks_mean<-aggregate(data_skinks[,c(14,15,17,18)],by=list(data_skinks$snakes),function(x) mean(x,na.rm=T))
skinks_sd<-aggregate(data_skinks[,c(14,15,17,18)],by=list(data_skinks$snakes),function(x) sd(x,na.rm=T))
skinks<-cbind(skinks_mean,skinks_sd)
write.csv(geckos,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Figures/Fig_morphology/geckos_mean.csv")
write.csv(skinks_sex,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Figures/Fig_morphology/skinks_sex.csv")
...
bsma_geckos<-lm(log10(weight)~log10(svl),data=data_geckos)
bsma_geckos<-bsma_geckos$coefficients[2]/cor.test(log10(data_geckos$weight),log10(data_geckos$svl),method="pearson")$estimate
data_geckos$scaled_mass<-log10(data_geckos$weight)*(mean(log10(data_geckos$svl),na.rm=T)/log10(data_geckos$svl))^bsma_geckos
...
model_bodycond<-glmmTMB(scaled_mass~snakes*sex+(1|site),data=data_geckos,dispformula =  ~site+sex)
plot(simulateResiduals(model_bodycond,n=1000))
Anova(model_bodycond,type=2)
pairs(emmeans(model_bodycond,~snakes|sex))
```
...
library(glmmTMB)
library(gplots)
bsma_lizards<-lm(log10(weight)~log10(svl),data=data_lizards)
bsma_lizards<-bsma_lizards$coefficients[2]/cor.test(log10(data_lizards$weight),log10(data_lizards$svl),method="pearson")$estimate
data_lizards$scaled_mass<-log10(data_lizards$weight)*(mean(log10(data_lizards$svl),na.rm=T)/log10(data_lizards$svl))^bsma_lizards
...
model_bodycond<-glmmTMB(scaled_mass~snakes*sex+(1|site),data=data_lizards,dispformula =  ~site+sex)
plot(simulateResiduals(model_bodycond,n=1000))
Anova(model_bodycond,type=2)
pairs(emmeans(model_bodycond,~snakes|sex))
pairs(emmeans(model_bodycond,~sex|snakes))
```
...
library(glmmTMB)
library(gplots)
bsma_skinks<-lm(log10(weight)~log10(svl),data=data_skinks)
bsma_skinks<-bsma_skinks$coefficients[2]/cor.test(log10(data_skinks$weight),log10(data_skinks$svl),method="pearson")$estimate
data_skinks$scaled_mass<-log10(data_skinks$weight)*(mean(log10(data_skinks$svl),na.rm=T)/log10(data_skinks$svl))^bsma_skinks
...
model_bodycond<-glmmTMB(scaled_mass~snakes*sex+(1|site),data=data_skinks,dispformula =  ~site+sex)
plot(simulateResiduals(model_bodycond,n=1000))
Anova(model_bodycond,type=2)
pairs(emmeans(model_bodycond,~snakes|sex))
pairs(emmeans(model_bodycond,~sex|snakes))
...
pairs(emmeans(glm_abundance,~sex|snakes))
...
lizards_mean<-aggregate(data_lizards[,c(34)],by=list(data_lizards$snakes,data_lizards$sex),function(x) mean(x,na.rm=T))
lizards_sd<-aggregate(data_lizards[,c(34)],by=list(data_lizards$snakes,data_lizards$sex),function(x) sd(x,na.rm=T))
lizards_bodycond<-cbind(lizards_mean,lizards_sd)
lizards_mean<-aggregate(data_lizards[,c(28)],by=list(data_lizards$snakes,data_lizards$sex),function(x) mean(x,na.rm=T))
lizards_sd<-aggregate(data_lizards[,c(28)],by=list(data_lizards$snakes,data_lizards$sex),function(x) sd(x,na.rm=T))
lizards_mites<-cbind(lizards_mean,lizards_sd)
geckos_mean<-aggregate(data_geckos[,c(28,31)],by=list(data_geckos$snakes,data_geckos$sex),function(x) mean(x,na.rm=T))
geckos_sd<-aggregate(data_geckos[,c(28,31)],by=list(data_geckos$snakes,data_geckos$sex),function(x) sd(x,na.rm=T))
geckos<-cbind(geckos_mean,geckos_sd)
skinks_mean<-aggregate(data_skinks$scaled_mass,by=list(data_skinks$snakes,data_skinks$sex),function(x) mean(x,na.rm=T))
skinks_sd<-aggregate(data_skinks$scaled_mass,by=list(data_skinks$snakes,data_skinks$sex),function(x) sd(x,na.rm=T))
skinks<-cbind(skinks_mean,skinks_sd)
write.csv(lizards_bodycond,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Figures/Fig_condition/lizards_bodycond.csv")
write.csv(lizards_mites,file="E:/Research/Projects/A_2019_CSIC_EvoImpLamp/Communication/Paper/Figures/Fig_condition/lizards_mites.csv")
...
lizards_n<-aggregate(data_lizards[,c(8:16,28,34)],by=list(data_lizards$snakes,data_lizards$sex),function(x) length(complete.cases(x)))
...
skinks_n<-aggregate(data_skinks[,c(8:23,27,42)],by=list(data_skinks$snakes,data_skinks$sex),function(x) length(complete.cases(x)))
...
geckos_n<-aggregate(data_geckos[,c(8:15,28,30,31)],by=list(data_geckos$snakes,data_geckos$sex),function(x) length(complete.cases(x)))
